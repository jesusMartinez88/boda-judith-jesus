        ...
        try {
            ...
            if (error instanceof Error) {
                if (error.name === 'NotSupportedError') {
                    throw new Error('No se encontró el archivo de música. Verifica que existe en public/music/background-music.mp3');
                } else if (error.name === 'NotAllowedError') {
                    throw new Error('El navegador bloqueó la reproducción. Haz clic en el botón para reproducir.');
                }
            }
            throw error;
        }
