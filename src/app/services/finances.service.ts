...
export class FinancesService {
    ...
    async createFinance(record: FinanceRecord): Promise<any> {
        try {
            const result = await firstValueFrom(this.http.post(this.apiUrl, record));
    ...
    async updateFinance(id: number, record: Partial<FinanceRecord>): Promise<any> {
        try {
            const result = await firstValueFrom(this.http.patch(`${this.apiUrl}/${id}`, record));
            await this.loadFinances(); // Refresh list
            return result;
        } catch (error) {
    ...
    async deleteFinance(id: number): Promise<any> {
        try {
